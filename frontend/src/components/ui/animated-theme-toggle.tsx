...
function Button({
  ...
  return (
    <button
      className={cn(
        'inline-flex items-center justify-center rounded-md border border-[rgba(184,134,11,0.24)] bg-[rgba(255,255,255,0.78)] text-sm font-medium text-[#5f5648] ring-offset-background transition-colors hover:bg-[rgba(255,249,235,0.95)] hover:text-[#b8860b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d4af37] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:border-white/10 dark:bg-white/6 dark:text-[#d8cebf] dark:hover:bg-white/12 dark:hover:text-[#f0d78d]',
        className
      )}
      disabled={disabled}
      onClick={onClick}
...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  ...
  if (!mounted) {
    return (
      <Button className={cn('p-2', className)} disabled>
